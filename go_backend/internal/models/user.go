...
func (u *User) ToProfile() *UserProfile {
	...
	// Build hierarchical roles array: each role includes all lower roles
	var roles []string
	switch role {
	case "admin":
		roles = []string{"admin", "developer", "support", "user"}
	case "developer":
		roles = []string{"developer", "support", "user"}
	case "support":
		roles = []string{"support", "user"}
	default:
		roles = []string{"user"}
	}
